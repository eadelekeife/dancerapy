...
const VideosPage = props => {
        ...
        try {
            ...
            if (userPurchase.data.statusMessage === "success") {
                ...
                userPurchase.data.message.filter(transactions => {
                    if (transactions.status === "completed") completedTransactions.push(transactions);
                })
    ...
    const data = walletData.map((plan, index) => {
        return {
            ...
            id: <p>{index + 1}</p>,
            name: <p>{plan?.ref}</p>,
            walletBalance: <p>{plan?.walletBalance}</p>,
            status: <p className={plan.type === "credit" ? 'bg-primary' : 'bg-danger'}>{plan?.type}</p>,
